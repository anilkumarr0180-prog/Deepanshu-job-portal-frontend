...
export function resolveExactLocality(
  ...
  const finalCity =
    city ||
    (state && state.toLowerCase() !== "punjab"
      ? state
      : "Mohali");
  ...
  if (
    area &&
    finalCity &&
    area.toLowerCase() !== finalCity.toLowerCase()
  ) {
    formattedName = `${area}, ${finalCity}`;
  } else if (
    finalCity &&
    state &&
    finalCity.toLowerCase() !== state.toLowerCase()
  ) {
...
const POPULAR_CITIES: CityEntry[] = [
  ...
  },
  {
    ...
    aliases: ["jalandhar", "jullundhur"],
  },
  {
    name: "Amritsar",
    state: "Punjab",
    label: "Amritsar, Punjab",
    aliases: ["amritsar", "ambarsar"],
  },
  {
    name: "Mumbai",
    state: "Maharashtra",
...
> = ({
  ...
  onSelectLocation,
    ...
    const [gpsPermissionState, setGpsPermissionState] =
      useState<
        ...
        try {
          const res = await fetch(
            `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(
              query
            )}&addressdetails=1&limit=6&countrycodes=in,us,gb,ca,ae`
          );
    ...
    const handleSelect = (
      ...
      toast.success(
        `📍 Location set to ${detail.formattedName}`
      );

      onClose();
    };

    const handleSelectCityName = (
      cityName: string,
      stateName: string = "",
      labelType: LocationDetail["labelType"] =
        "CUSTOM"
    ) => {
    ...
    // Enterprise Hybrid Location Auto-Detection Strategy
    const handleAutoDetectGPS = useCallback(
      async () => {
        setIsLocating(true);
        ...
        if (
          ...
          navigator.geolocation.getCurrentPosition(
            ...
            async (err) => {
              console.warn(
                "GPS Permission or Hardware Timeout. Triggering Zero-Click IP Detection:",
                err.message
              );
              ...
              try {
                ...
                if (
                  ...
                  toast.success(
                    `📍 Set to ${ipRes.data.data.formattedName} (approx. via network IP)`
                  );
                } else {
                  handleSelectCityName(
                    "Mohali",
    ...
    const mapEmbedUrl =
      currentLoc?.latitude &&
        ...
        ? `https://www.openstreetmap.org/export/embed.html?bbox=${currentLoc.longitude - 0.02
        }%2C${currentLoc.latitude - 0.02
        }%2C${currentLoc.longitude + 0.02
        }%2C${currentLoc.latitude + 0.02
        }&layer=mapnik&marker=${currentLoc.latitude
        }%2C${currentLoc.longitude}`
        : `https://www.openstreetmap.org/export/embed.html?bbox=76.65%2C30.65%2C76.75%2C30.75&layer=mapnik&marker=30.7046%2C76.7179`;

    // Keep the modal outside the dashboard/header DOM tree.
    // The overlay itself owns the viewport and the backdrop owns the close action.
    return createPortal(
      <div
        className="fixed inset-0 z-[1000] isolate flex h-screen w-screen items-center justify-center p-4"
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        {/* Modal Container */}
        <div
          className="relative z-10 w-full max-w-lg max-h-[90vh] overflow-y-auto scrollbar-none overscroll-contain rounded-3xl border border-slate-100 bg-white p-6 shadow-2xl transition-all sm:p-8 animate-enter"
          onMouseDown={(event) =>
        ...
        >
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">
                  Find jobs & talent near your exact area
                </p>
              </div>
            </div>
            ...
            <div className="mt-4 flex items-start gap-2.5 rounded-2xl border border-amber-200 bg-amber-50 p-3.5 text-xs text-amber-900">
              ...
              <div>
                <p className="font-bold">
                  Browser Location Access Blocked
                </p>

                <p className="mt-0.5 text-[11px] text-amber-700">
                  Click the 🔒 lock icon in your browser
                  address bar next to{" "}
                  <strong>localhost</strong>, set Location
                  to <strong>Allow</strong>, then try Auto
                  Detect.
                </p>
              </div>
            </div>
          ...
          <div className="mt-4 flex rounded-2xl bg-slate-100 p-1">
            <button
              ...
              className={`flex flex-1 items-center justify-center gap-2 rounded-xl py-2 text-xs font-bold transition-all ${activeTab === "SEARCH"
            ...
            <button
              ...
              className={`flex flex-1 items-center justify-center gap-2 rounded-xl py-2 text-xs font-bold transition-all ${activeTab === "MAP"
                ? "bg-white text-[#3C65F5] shadow-sm"
                : "text-slate-600 hover:text-slate-900"
                }`}
            >
            ...
            <>
              ...
              <div className="relative mt-4">
                ...
                <input
                  ...
                  className="w-full rounded-2xl border border-slate-200 bg-slate-50/50 py-3.5 pl-12 pr-10 text-sm font-medium text-slate-800 outline-none transition placeholder:text-slate-400 focus:border-[#3C65F5] focus:bg-white focus:ring-4 focus:ring-blue-500/10"
                />
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-bold tracking-tight">
                        {isLocating
                          ? "Detecting Location (GPS / Network)..."
                          : "🎯 Use Current Location"}
                      </p>

                      <p className="text-[11px] text-blue-100">
                        Hybrid GPS + Server IP
                        auto-detection
                      </p>
                    </div>
                  </div>
              ...
              {!searchTerm && (
                <div className="mt-4 flex gap-2 overflow-x-auto pb-1">
                  <button
                    ...
                    className="flex items-center gap-1.5 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs font-semibold text-slate-700 transition hover:border-blue-200 hover:bg-blue-50 hover:text-[#3C65F5]"
                  >
              ...
              {/* Dynamic Location Search Suggestions */}
              <div className="mt-4 max-h-72 overflow-y-auto scrollbar-none overscroll-contain pr-1">
                {searchTerm ? (
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      {isSearchingApi && (
                        <span className="inline-flex items-center gap-1 text-xs text-[#3C65F5]">
                          ...
                          Searching global maps...
                        </span>
                      )}
                    </div>

                    {matchedPreindexed.length >
                      0 && (
                        <div className="space-y-1">
                          {matchedPreindexed.map(
                            (city) => (
                              <button
                                key={city.label}
                                type="button"
                        ...
                        {apiResults.map(
                          (res, idx) => (
                            <button
                              ...
                              className="flex w-full items-center justify-between rounded-xl border border-transparent px-3 py-2.5 text-left text-sm font-medium text-slate-700 transition hover:border-slate-200 hover:bg-slate-50 hover:text-slate-900"
                            >
                    ...
                    <button
                      ...
                      className="flex w-full items-center justify-center gap-2 rounded-xl bg-slate-900 py-3 text-xs font-bold text-white shadow-sm transition hover:bg-slate-800 active:scale-[0.99]"
                    >
                        ...
                        (city) => {
                          ...
                          return (
                            <button
                              ...
                              className={`flex items-center justify-between rounded-xl border px-3 py-2.5 text-left text-xs font-semibold transition-all ${isSelected
                            ...
                            >
                              ...
                              {isSelected && (
                                <Check className="h-3.5 w-3.5 shrink-0 text-[#3C65F5]" />
                              )}
                            </button>
                          );
                        }
          ...
          {activeTab === "MAP" && (
            <div className="mt-4 space-y-4">
              <div className="relative overflow-hidden rounded-2xl border border-slate-200 bg-slate-100 shadow-inner">
                ...
                <div className="absolute left-3 top-3 rounded-xl border border-white/40 bg-white/90 px-3 py-1 text-[11px] font-bold text-slate-800 shadow-xs backdrop-blur-md">
                  📍{" "}
                  {currentLoc?.formattedName ||
                    "Mohali, Punjab"}
                </div>
