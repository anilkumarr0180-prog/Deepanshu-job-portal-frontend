...
export default function AdminUsersPage() {
  ...
  // Selected user for modal confirmation
  ...
  const mappedUsers: AdminUser[] = (data?.items ?? []).map((u) => ({
    id: u._id,
    name: u.name,
    email: u.email,
    role: u.role,
  ...
  return (
    <div className="space-y-6">
      <Toolbar
        title="Users"
        description="Manage platform accounts, role permissions, and access status."
        searchValue={search}
        onSearchChange={(val) => {
          setSearch(val);
          setPage(1);
      ...
      <div className="flex flex-wrap items-center gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <div className="flex items-center gap-2">
          ...
          <FilterDropdown
            ...
            options={["All", "candidate", "recruiter", "admin"]}
          />
        </div>
        <div className="flex items-center gap-2">
      ...
      {isError ? (
        <div className="rounded-xl border border-red-200 bg-red-50 p-6 text-center text-sm text-red-600">
          <p className="mb-3">Failed to load users.</p>
          <button
            type="button"
            onClick={() => {
              void refetch();
        ...
        <EmptyState
          ...
          description="No users matched your filter or search query."
        />
      )}
      ...
      {userToBlock && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-xs p-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl space-y-4">
            <h3 className="text-lg font-semibold text-slate-900">Block User Account</h3>
      ...
      {userToUnblock && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-xs p-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl space-y-4">
            <h3 className="text-lg font-semibold text-slate-900">Unblock User Account</h3>
            <p className="text-sm text-slate-600">
