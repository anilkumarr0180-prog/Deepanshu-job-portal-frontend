...
import type { CandidateBlogItem, CandidateBlogStatus } from "../types/candidate-blog.types";
...
export default function CandidateBlogsPage() {
  ...
  const [blogToDelete, setBlogToDelete] = useState<CandidateBlogItem | null>(null);

  // Queries & Mutations
  const { data: categories } = useBlogCategories();

  const queryStatus: CandidateBlogStatus | "all" =
  ...
  const deleteMutation = useDeleteMyBlog();
  const publishMutation = usePublishMyBlog();
  ...
  const handlePublish = (blog: CandidateBlogItem) => {
    publishMutation.mutate(blog._id);
  };

  const handleUnpublish = (blog: CandidateBlogItem) => {
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2.5">
              <span className="inline-flex items-center gap-1.5 rounded-full bg-white/10 px-3 py-1 text-xs font-bold text-blue-300 backdrop-blur-md border border-white/10">
                <Sparkles className="h-3.5 w-3.5 text-amber-400" /> Candidate Author Studio
              </span>
        ...
        <div className="flex flex-col items-center justify-center rounded-3xl border border-dashed border-slate-200 bg-white p-12 text-center dark:border-slate-800 dark:bg-[#0B1220]">
          ...
          <p className="mt-1 max-w-sm text-xs text-slate-500 dark:text-slate-400">
            {search || statusFilter !== "All" || categoryFilter !== "All Categories"
              ...
              : "You haven't written any blogs yet. Share your experience and knowledge!"}
          </p>
          <Link
            to="/candidate/blogs/create"
                        ...
                        <td className="px-5 py-4">
                          <div className="flex items-center gap-3.5">
                            ...
                            <div className="min-w-0 max-w-sm">
                              <Link
                                to={`/candidate/blogs/${blog._id}/edit`}
