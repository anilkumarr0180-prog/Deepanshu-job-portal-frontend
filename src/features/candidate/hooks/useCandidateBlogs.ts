...
export function useMyBlogs(params?: CandidateBlogQueryParams) {
  return useQuery({
...
export function useMyBlog(id: string | undefined) {
  return useQuery({
    queryKey: ["my-blog", id],
    queryFn: async () => {
...
export function useCreateMyBlog() {
  ...
  return useMutation({
    mutationFn: (payload: CreateCandidateBlogPayload) => createMyBlog(payload),
    onSuccess: (res) => {
      toast.success(res.message || "Blog created successfully.");
      void queryClient.invalidateQueries({ queryKey: ["my-blogs"] });
...
export function useUpdateMyBlog() {
  ...
  return useMutation({
    ...
    onSuccess: (res, variables) => {
      ...
      void queryClient.invalidateQueries({ queryKey: ["my-blogs"] });
      void queryClient.invalidateQueries({ queryKey: ["my-blog", variables.id] });
      void queryClient.invalidateQueries({ queryKey: ["public-blogs"] });
...
export function usePublishMyBlog() {
  ...
  return useMutation({
    ...
    onSuccess: (res, id) => {
      ...
      void queryClient.invalidateQueries({ queryKey: ["my-blogs"] });
      void queryClient.invalidateQueries({ queryKey: ["my-blog", id] });
      void queryClient.invalidateQueries({ queryKey: ["public-blogs"] });
      void queryClient.invalidateQueries({ queryKey: ["public-blog"] });
      void queryClient.invalidateQueries({ queryKey: ["featured-blogs"] });
      void queryClient.invalidateQueries({ queryKey: ["trending-blogs"] });
...
export function useUnpublishMyBlog() {
  const queryClient = useQueryClient();
