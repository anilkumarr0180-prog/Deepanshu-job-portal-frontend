...
export default function RecruiterNotificationsPage() {
  ...
  // Enterprise Feature: Auto mark all as read on entering Recruiter Notification Center
  useEffect(() => {
    if (unreadCount > 0) {
      void markAllAsRead();
    }
  ...
  const handleItemClick = async (
    ...
    const targetLink = normalizeNotificationLink(link, "recruiter", type);
    if (targetLink) {
      navigate(targetLink);
    }
  };
    ...
    <div className="space-y-6 max-w-5xl mx-auto pb-12">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              Real-time activity feed and system alerts for your hiring and networking workspace.
            </p>
          </div>
      ...
      {isLoading ? (
        <div className="rounded-2xl border border-slate-200 bg-white p-12 text-center shadow-xs">
          ...
          <p className="mt-3 text-sm font-medium text-slate-500">Loading notifications feed...</p>
        </div>
          ...
          {filteredNotifications.map((notification) => {
            ...
            return (
              <div
                key={itemId}
                onClick={() => {
                  void handleItemClick(itemId, notification.link, isRead, notification.type);
                }}
        ...
        <div className="rounded-2xl border border-dashed border-slate-200 bg-white p-12 text-center shadow-xs">
          ...
          <p className="mt-1 text-sm text-slate-500">
            You are all caught up! There are no notifications under this filter right now.
          </p>
        </div>
      )}
    </div>
