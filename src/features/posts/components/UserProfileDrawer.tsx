...
export default function UserProfileDrawer() {
  ...
  const handleReportUser = () => {
    setMoreMenuOpen(false);
    toast.success("Thank you. We have received your report for review.");
  };
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden bg-slate-950/70 backdrop-blur-md transition-opacity animate-in fade-in duration-200">
      <div className="absolute inset-y-0 right-0 flex max-w-full pl-3 sm:pl-10">
        <div className="w-screen max-w-lg bg-[#F8FAFC] shadow-2xl flex flex-col h-full animate-in slide-in-from-right duration-300 border-l border-slate-200/80">
          ...
          {/* ── Top Cover Banner with Modern Mesh & Glowing Accents ── */}
          <div className="relative h-40 sm:h-44 w-full bg-gradient-to-br from-[#0F172A] via-[#1E293B] to-[#3C65F5] shrink-0 overflow-hidden">
            {/* Glowing Accent Orb */}
            <div className="absolute -top-12 -right-12 h-44 w-44 rounded-full bg-blue-500/30 blur-3xl" />
            <div className="absolute bottom-0 left-10 h-32 w-32 rounded-full bg-indigo-500/20 blur-2xl" />
          ...
          <div className="relative px-6 pb-5 -mt-16 bg-white border-b border-slate-200/80 shadow-xs shrink-0 rounded-t-3xl">
            
            {/* Avatar & Status Row */}
            <div className="flex items-end justify-between gap-4">
              
              {/* Crisp Elevated Avatar with Ring & Status Glow */}
              <div className="relative -mt-4">
                <div className="p-1 rounded-full bg-white shadow-2xl ring-4 ring-white">
                  <div className="rounded-full overflow-hidden ring-2 ring-blue-500/20">
                    <UserAvatar
                ...
                <span className="absolute bottom-2 right-2 h-4 w-4 rounded-full bg-emerald-500 ring-3 ring-white shadow-sm" />
              </div>

              {/* Status Pill Badges */}
              <div className="flex flex-wrap items-center gap-1.5 pb-1">
                <span
                  className={`inline-flex items-center gap-1 rounded-full px-3 py-1 text-xs font-extrabold tracking-wide border shadow-2xs ${
                    isRecruiter
                ...
                <span className="rounded-full px-2.5 py-1 text-xs font-bold capitalize bg-slate-100 text-slate-700 border border-slate-200/80">
                  {profile.role || "Member"}
                </span>
              </div>
            </div>
            ...
            <div className="mt-3.5 space-y-1.5">
              <div className="flex items-center gap-2">
                ...
                <div className="flex items-center text-[#3C65F5]" title="Verified Member">
                  <BadgeCheck className="h-5 w-5 fill-blue-50" />
                </div>
              </div>

              <p className="text-xs sm:text-sm font-medium text-slate-600 leading-relaxed max-w-md">
                {headline}
              </p>
              ...
              <div className="flex flex-wrap items-center gap-2 pt-1">
                {locationStr && (
                  <span className="inline-flex items-center gap-1 rounded-lg bg-slate-50 px-2.5 py-1 text-xs font-semibold text-slate-600 border border-slate-200/80">
                    <MapPin className="h-3.5 w-3.5 text-slate-400 shrink-0" />
                    <span>{locationStr}</span>
                  </span>
                )}
                <span className="inline-flex items-center gap-1 rounded-lg bg-blue-50/80 px-2.5 py-1 text-xs font-bold text-[#3C65F5] border border-blue-100">
            ...
            {!isOwnProfile && (
              <div className="mt-4 pt-3 border-t border-slate-100 flex items-center gap-2.5">
                <ConnectionButton
                  ...
                  className="flex-1 min-w-[140px] shadow-sm font-bold"
                />

                <button
                  type="button"
                  onClick={handleStartMessage}
                  className="inline-flex items-center justify-center gap-1.5 rounded-xl border border-slate-200 bg-white px-4 py-2 text-xs sm:text-sm font-bold text-slate-700 shadow-2xs hover:bg-blue-50 hover:text-[#3C65F5] hover:border-blue-200 transition-all cursor-pointer"
                >
                  <MessageSquare className="h-4 w-4 text-[#3C65F5]" />
                  <span>Message</span>
                </button>
                ...
                <div ref={moreMenuRef} className="relative">
                  <button
                    ...
                    className={`inline-flex items-center justify-center rounded-xl border p-2.5 text-slate-600 transition-all cursor-pointer shadow-2xs ${
                      moreMenuOpen
                        ? "border-[#3C65F5] bg-blue-50 text-[#3C65F5] ring-2 ring-blue-100"
                  ...
                  {moreMenuOpen && (
                    <div className="absolute right-0 top-full mt-2 w-56 rounded-2xl border border-slate-200 bg-white p-2 shadow-2xl z-50 animate-in fade-in zoom-in-95 duration-150">
                      <button
                        ...
                        className="flex w-full items-center gap-2.5 rounded-xl px-3 py-2 text-xs font-bold text-slate-700 hover:bg-blue-50 hover:text-[#3C65F5] transition text-left cursor-pointer"
                      >
                        {copiedLink ? (
            ...
            <div className="mt-4 pt-1">
              <div className="flex rounded-xl bg-slate-100/90 p-1 border border-slate-200/60">
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-1.5 py-2 text-xs font-extrabold rounded-lg transition-all cursor-pointer ${
                    activeTab === "about"
          ...
          {/* ── Scrollable Body Content with Neo-Modern Cards ── */}
            ...
            {activeTab === "about" ? (
              isLoadingProfile ? (
                ...
                <div className="space-y-5">
                  {/* 1. About / Bio Card */}
                  <div className="group rounded-2xl border border-slate-200/90 bg-white p-5 shadow-xs hover:shadow-md transition-all duration-200 space-y-3">
                    <div className="flex items-center gap-2.5">
                      <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-blue-50 text-[#3C65F5] border border-blue-100">
                    ...
                    <div className="group rounded-2xl border border-slate-200/90 bg-white p-5 shadow-xs hover:shadow-md transition-all duration-200 space-y-3">
                      ...
                      <div className="flex flex-wrap gap-2 pt-1">
                        {profile.skills.map((skill: string, sIdx: number) => (
                          <span
                            key={sIdx}
                            className="rounded-xl bg-slate-50 px-3.5 py-1.5 text-xs font-bold text-slate-700 border border-slate-200/80 shadow-2xs hover:border-[#3C65F5] hover:text-[#3C65F5] hover:bg-blue-50/50 hover:scale-105 transition-all duration-150 cursor-default"
                          >
                            {skill}
                          </span>
                        ))}
                  ...
                  {profile.experience && profile.experience.length > 0 && (
                    <div className="group rounded-2xl border border-slate-200/90 bg-white p-5 shadow-xs hover:shadow-md transition-all duration-200 space-y-4">
                      <div className="flex items-center gap-2.5">
                        ...
                        {profile.experience.map((exp: any, idx: number) => (
                          <div key={idx} className={idx > 0 ? "pt-4" : ""}>
                            <div className="flex items-start justify-between gap-2">
                              <div>
                                <h5 className="text-sm font-extrabold text-slate-900">{exp.title}</h5>
                                <p className="text-xs font-bold text-[#3C65F5] mt-0.5">{exp.company}</p>
                              </div>
                              {exp.current && (
                  ...
                  {isRecruiter && typeof profile.companyId === "object" && profile.companyId?.name && (
                    <div className="group rounded-2xl border border-slate-200/90 bg-white p-5 shadow-xs hover:shadow-md transition-all duration-200 space-y-3">
                      ...
                      <div className="flex items-center gap-3.5 pt-1">
                        ...
                        <div>
                          <h5 className="text-sm font-extrabold text-slate-900">{profile.companyId.name}</h5>
                      ...
                      <div className="space-y-3 pt-1">
                        {profile.education.map((edu: any, idx: number) => (
                          <div key={idx} className="flex items-start gap-3">
                            ...
                            <div>
                              <h5 className="text-sm font-extrabold text-slate-900">{edu.institution}</h5>
                  ...
                  <div className="rounded-2xl border border-slate-200/90 bg-white p-5 shadow-xs space-y-3">
                    <div className="flex items-center gap-2">
                      <Mail className="h-4 w-4 text-[#3C65F5]" />
                      <h4 className="text-xs font-extrabold uppercase tracking-wider text-slate-900">
