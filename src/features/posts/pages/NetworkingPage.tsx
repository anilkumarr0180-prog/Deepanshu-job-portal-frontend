...
type NetworkTab = "feed" | "saved" | "grow" | "invitations" | "connections";
...
function NetworkingContent() {
  ...
  return (
    <div className="flex flex-col lg:h-[calc(100vh-7.5rem)] lg:overflow-hidden gap-5">
        ...
        <aside className="hidden lg:block space-y-5 lg:h-full lg:overflow-y-auto lg:overscroll-contain lg:pr-2 custom-scrollbar pb-8">
            ...
            <div className="rounded-2xl border border-slate-200/90 bg-white p-5 shadow-xs transition hover:border-slate-300">
              ...
              <div className="mt-4 border-t border-slate-100 pt-3.5 flex items-center justify-between">
                <button
                  type="button"
                  onClick={handleOpenOwnProfile}
                  className="flex items-center justify-between text-xs font-semibold text-blue-600 hover:text-blue-700 transition group cursor-pointer"
                >
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <ExternalLink className="h-3 w-3 text-slate-400 group-hover:text-blue-600 shrink-0 opacity-0 group-hover:opacity-100 transition" />
                      </div>
          ...
          <div className="rounded-2xl border border-slate-200/90 bg-white p-5 shadow-xs space-y-3">
            ...
            <div className="space-y-1.5">
              <Link
                to={jobsRoute}
                className="flex items-center gap-2.5 rounded-xl px-3 py-2 text-xs font-medium text-slate-700 hover:bg-slate-50 hover:text-blue-600 transition"
              >
      ...
      {/* Tab: Saved Posts */}
      {activeTab === "saved" && (
        <div className="lg:flex-1 lg:min-h-0 lg:overflow-y-auto custom-scrollbar pb-8 max-w-3xl mx-auto w-full">
          <SavedPostsFeed onExploreFeed={() => handleTabChange("feed")} />
        </div>
      )}
